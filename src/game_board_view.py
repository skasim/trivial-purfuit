...
def board_square_click(players, names, turn, button, tk_label):
    """
    ...
    if button['text'] == ' ' and not (button['text'] in board_labels):
        ...
        display_question(Color.return_color_from_hex(button['highlightbackground']), tk_label)
        ...
        display_question(Color, tk_label)
...
def display_question(color_type, tk_label):
    """
    ...
    red_questions = [
        ...
        "Who was the author of the original draft of the Declaration of Independence?",
    ]

    white_questions = [
        "What year did the first Continental Congress meet on?",
        "When was the first draft of the Declaration of Independence signed?"
    ]

    blue_questions = [
        "What state did Benjamin Franklin first arrive at to seek his fortune?",
        "Where did the first Continental Congress convene?"
    ]

    green_questions = [
        "When is Independence Day celebrated in the United States?",
        "What is the most popular form of celebration on Independence Day?"
    ]

    if color_type == Color.RED.color:
        tk_label.configure(text=random.choice(red_questions))
    elif color_type == Color.WHITE.color:
        tk_label.configure(text=random.choice(white_questions))
    elif color_type == Color.BLUE.color:
        tk_label.configure(text=random.choice(blue_questions))
    elif color_type == Color.GREEN.color:
        tk_label.configure(text=random.choice(green_questions))
    else:
